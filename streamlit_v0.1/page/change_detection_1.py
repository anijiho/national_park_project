...
def app():
    ...
    # 자세한 안내 메시지를 표시합니다.
    st.text('''
        변화 탐지 기간은 최대 9개월 간격으로 설정할 수 있습니다. 
        이는 대량의 위성 데이터 처리 시 메모리 제한을 방지하기 위한 조치입니다. 
        더 긴 기간의 데이터를 분석하고자 할 경우, 여러 번의 작은 기간으로 나누어 분석해 주세요. 
    ''')
    ...
    with tab1:
        ...
        if submitted:
            ...
            with col2:
                ...
                st.markdown("""
                        **색상 코드**\n 
                        양의 변화(:red[빨간색]), 음의 변화(:blue[파란색]), 극적인 변화(:orange[노란색])
                        """)
