...
def get_dynamic_world_data(aoi, date):
    ...
    return built_mean, crops_mean, water_mean
...
# 사용자 입력 처리를 위한 함수
def process_user_input(lat, lon):
    ...
    if nearest_date_vh is None or nearest_date_dw is None:
        ...
        period_built_mean, period_crops_mean, period_water_mean = get_dynamic_world_data(aoi, current_date)
        ...
        recent_built_mean, recent_crops_mean, recent_water_mean = get_dynamic_world_data(aoi, nearest_date_dw)

        # 결과를 표 형태로 표시
        st.write("과거 평균적인 경향성")
        st.table({
            'Mean VH (Period)': [period_mean_vh.get('VH', None)],
            'Built_mean (Period)': [period_built_mean],
            'Crops_mean (Period)': [period_crops_mean],
            'Water_mean (Period)': [period_water_mean],
        ...
        st.write("최근 값")
        st.table({
            '가장 가까운 날짜': [nearest_date_vh.strftime('%Y-%m-%d') if nearest_date_vh else 'None'],
            'Mean VH (Recent)': [recent_mean_vh.get('VH', None) if recent_mean_vh else 'None'],
            '가장 가까운 날짜': [nearest_date_dw.strftime('%Y-%m-%d') if nearest_date_dw else 'None'],
            'Built (Recent)': [recent_built_mean if recent_built_mean else 'None'],
            'Crops (Recent)': [recent_crops_mean if recent_crops_mean else 'None'],
            'Water (Recent)': [recent_water_mean if recent_water_mean else 'None']
        })
